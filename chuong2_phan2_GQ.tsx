...
const P: number = 15.99; // Công suất (kW)

const T1: number = 145252.23; // Moment xoắn đầu vào (Nmm)
const T2: number = 1364084.03; // Moment xoắn đầu ra (Nmm)

const n1: number = 978.33; // Số vòng quay đầu vào (vòng/phút)
const n2: number = 87.66; // Số vòng quay đầu ra (vòng/phút)

const t2: number = 78840; // Số giờ trong 9 năm

const u_thucTe: number = 11.16; // Tỷ số truyền
...
const ungSuatUonChoPhep_oF = Number(ungSuatUonChoPhep(
  coTinh.gioiHanChay_och!,
  coTinh.gioiHanBen_ob!,
  N_FE
));
...
const khoangCachTruc_aw = Number(tinhAw(heSoDuongKinh_q, soRangBanhVit_z2, 165, T2, 1.25).toFixed(2));
...
const ungSuatTiepXucChoPhep_oHtinhLai = Number(tinhLaiUngSuatTiepXucChoPhep(vanTocTruot_Vs, vanTocTruotXacDinh_VsXacDinh).toFixed(2));
...
const yF2List = [1.80, 1.76, 1.71, 1.64, 1.61, 1.55, 1.48, 1.45, 1.40, 1.34, 1.30, 1.27, 1.24];
...
function timYF2(zv2: number): number {
    ...
    for (let i = 1; i < zv2List.length; i++) {
        let khoangCach = Math.abs(zv2 - zv2List[i]);
...
const kiemNghiemDoBenUonVitOf = (T2: number, yf2: number, kf: number, d2: number, b2: number, m: number): number => {
  const of = (1.2*T2*yf2*kf)/(d2*b2*m);
  return of; 
}

const doBenUonVit_oF = Number(kiemNghiemDoBenUonVitOf(T2, YF2, Kf, duongKinhVongChia_d2, chieuRongCuaBanhVit_b2, modun_m).toFixed(2));
...
const nhietDoDau_t1 = Math.ceil(tinhT1(nhietDoMoiTruongXungQuanh_t0, P, hieuSuatSoBo_n, heSoToaNhiet_Kt, dienTichBeMatThoatNhiet_A, heSoThoatNhietQuaBeMay_psi));
...
console.log(`Lực vòng trục vít bằng lực dọc trục bánh vít Ft1 = Fa2 = ${lucVongTrucVit_Ft1} = ${lucDocTrucBanhVit_Fa2} (N)`);
...
console.log(`Lực vòng bánh vít bằng lực dọc trục của trục vít Ft2 = Fa1 = ${lucVongTrucVit_Ft2} = ${lucDocTrucBanhVit_Fa1} (N)`);
...
console.log(`Lực hướng tâm trục vít và bánh vít Fr1 = Fr2 = ${lucHuongTamTrucVit_Fr1} = ${lucHuongTamBanhVit_Fr2} (N)`);
...
const Mf = Number(tinhMf(lucVongTrucVit_Ft1, l, lucHuongTamTrucVit_Fr1, lucDocTrucBanhVit_Fa1, duongKinhVongChia_d1).toFixed(2));
...
const momentQuanTinhTuongDuongMatCatTrucVit_Je = Number(tinhJe(duongKinhVongDinh_da1, duongKinhVongDay_df1).toFixed(2));
...
const doCungCuaTrucVit_y = Number(tinhDoCungY(l, lucHuongTamTrucVit_Fr1, lucVongTrucVit_Ft1, E, momentQuanTinhTuongDuongMatCatTrucVit_Je).toFixed(5));
...
const P: number = 9.4548; // Công suất (kW)

const T1: number = 174495.2284; // Moment xoắn đầu vào (Nmm)
const T2: number = 1258356.026; // Moment xoắn đầu ra (Nmm)

const n1: number = 486.67; // Số vòng quay đầu vào (vòng/phút)
const n2: number = 58.1254; // Số vòng quay đầu ra (vòng/phút)

const t2: number = 78840; // Số giờ trong 9 năm

const u_thucTe: number = 8.3727; // Tỷ số truyền
...
// const khoangCachTruc_aw = tinhAw(heSoDuongKinh_q, soRangBanhVit_z2, ungSuatTiepXucChoPhep_oH, T2, heSoTrongTaiTinh_Kh).toFixed(2);
const khoangCachTruc_aw = Number(tinhAw(heSoDuongKinh_q, soRangBanhVit_z2, ungSuatTiepXucChoPhep_oH, T2, heSoTrongTaiTinh_Kh).toFixed(2));
console.log(`Khoảng cách trục aw theo độ bền tiếp xúc = ${khoangCachTruc_aw}`)
...
let ungSuatTiepXucChoPhep_oHtinhLai = Number(tinhLaiUngSuatTiepXucChoPhep(vanTocTruot_Vs, vanTocTruotXacDinh_VsXacDinh).toFixed(2));
